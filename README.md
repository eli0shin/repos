...
- **Persistent work contexts** - Open worktrees as tmux or Herdr Managed Workspaces
...
Set `config.workspaceManager` to `"tmux"` or `"herdr"` to select the Workspace Manager outside a managed environment. The existing `--tmux`, `--no-tmux`, and `--no-focus` flags control either provider.

See [USERGUIDE.md](./USERGUIDE.md) for complete documentation, including Workspace Manager selection and repo-local worktree setup examples.
